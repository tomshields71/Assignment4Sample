...

{
    ...
    if(this->isEmpty()) {
        ...
        while(tracer->getNext() != nullptr)


            tracer = tracer->getNext();

        tracer->setNext(nextNodePtr);
...

{
    ...
    bool canRemoveItem = !this->isEmpty();

    if(canRemoveItem)
    {

        int nodeToRemove = rand() % (this->itemCount - 1);
